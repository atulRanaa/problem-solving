...
int main() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        // prduct of (p ^ (x + 1) - 1)/(p - 1)
        sum *= ((binpow(p, x + 1, MOD) - 1 + MOD) * binpow(p - 1, MOD - 2, MOD)) % MOD;
        sum %= MOD;
        
        // When a new prime factor p^x arrives, its internal product contribution is:
        // pro = (pro ^ (x + 1) * (p ^ AP(x)) ^ (num of divisor % MOD - 1)) % MOD
        pro = binpow(pro, x + 1, MOD);
        pro *= binpow(binpow(p, (x * (x + 1))/2, MOD), num_div, MOD) % MOD;
        pro %= MOD;

        num_div = (num_div * (x + 1)) % (MOD - 1);
    }
