...
int main() {
    ...
    // burnside lemma
    // sum(c(k)/n) for all k in range(1, n), c(k) is combination remain unchanged when kth rotation applied

    // ans = sum (2 ^ (n * n - 1) / 4) for k (1 to 4)
    ...
    for(int i = 0; i < 4; i++) {
        ...
        if(i == 0) {
            ...
            p = (n & 1)?((n * n - 1)/4 + 1):(n*n)/4;
        } else {
            p = (n & 1)?((n * n - 1)/2 + 1):(n*n)/2;
        }

        a += binpow(2, p, MOD);
        a %= MOD;
    }
    ll b = binpow(4, MOD - 2, MOD);
    ll ans = (a * b) % MOD;
