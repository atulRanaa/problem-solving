...
int main() {
    ...
    const int N = 2e6;
    LiChaoStatic<ll, true> tree(0, N);

    vector<ll> dp(n, inf);
    for(int i = 0; i < n; i++) {
        ll result = tree.query(strength[i]);
        
        dp[i] = min(strength[i] * x, result);
        // for(int j = 0; j < i; j++) {
        //     dp[i] = min(dp[i], dp[j] + skill[j] * strength[i]);
        // }

        tree.add_line(skill[i], dp[i]);
    }
