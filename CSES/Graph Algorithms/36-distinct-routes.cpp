...
private:
    ...
    bool search(int v, int &t, vector<bool> &visited, vector<int> &path) {
        path.push_back(v);
        visited[v] = true;
        if(v == t)
            return true;
        ...
        for(auto& e: graph[v]) {
            if(!e.is_rev && e.cap == 0 && graph[e.to][e.rev].cap > 0 && !visited[e.to]) {
                e.cap = 1;
                if(search(e.to, t, visited, path))
                    return true;
                e.cap = 0;
            }
        }
...
public:
    ...
    void add_edge(int u, int v, ll cap) {
        graph[u].push_back({v, (int)graph[v].size(),     cap, false});
        graph[v].push_back({u, (int)graph[u].size() - 1, 0, true  });
    }

    // add undirected edge (both directions with full cap)
    void add_edge_undirected(int u, int v, ll cap) {
        graph[u].push_back({v, (int)graph[v].size(),     cap, false});
        graph[v].push_back({u, (int)graph[u].size() - 1, cap, false});
    }
    ...
    vector< vector<int> > edge_disjoint_paths(int s, int t) {
        vector< vector<int> > disjoint_path;
        // path is build with edge weight of 1
        bool f;
        do {
            vector< int > path;
            vector< bool> visited(n, false);
            f = search(s, t, visited, path);
            if(f) {
                disjoint_path.push_back(path);
            }
        } while(f);
...
int main() {
    ...
    loop (i, m) {
        ...
        --a;
        --b;
        maxflow.add_edge(a, b, 1);
    }

    maxflow.max_flow(0, n-1);
    auto ans = maxflow.edge_disjoint_paths(0, n-1);
