...
void dfs(int node, int prev, vector< vector<int> > &adj, vector< int > &visited, vector< int > &parent, vector< int > &path) {
    visited[node] = 1;
    ...
    for(int v: adj[node]) {
        ...
        if(visited[v] == 0) {
            parent[v] = node;
            dfs(v, node, adj, visited, parent, path);
        }
    }
...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        if(visited[i] == 0) {
            dfs(i, -1, adj, visited, parent, path);
