...
int main() {
    ...
    vector< vector<int> > up(n + 2, vector<int> (LOG, end_index));
    vector< vector<ll > > op(n + 2, vector<ll> (LOG, 0));

    for(int k = 0; k < LOG; k++) {
        // contribution = arr[k] * (nge[k] - k) - sum(arr[k, nge[k] - 1])
        for(int i = 1; i <= n; i++) {
            if(k == 0) {
                up[i][k] = nge[i];
                op[i][k] = arr[i] * (nge[i] - i) - (prefix[nge[i] - 1] - prefix[i - 1]);
            } else {
                up[i][k] = up[ up[i][k-1] ][k-1];
                op[i][k] = op[i][k-1] + op[up[i][k-1]][k-1];
            }
        }
    }

    auto lift = [&] (int l, int r) -> ll {
        ...
        if (l <= r) {
            ans += arr[l] * (ll)(r - l + 1) - (prefix[r] - prefix[l - 1]);
        }
