...
int main() {
    ...
    const int LOG = 20;
    const int N = 1e6;
    const int SENINAL = N + 1;
    vector< ll > suffix_min(n + 1, SENINAL);
    for(int i = n - 1; i >= 0; i--) {
        suffix_min[i] = min(arr[i].second, suffix_min[i + 1]);
    }

    vector< int > nt(N + 2, SENINAL);
    vector< vector<int> > up(N + 2, vector<int> (LOG, SENINAL));

    for(int i = 1; i <= N; i++) {
        int idx = lower_bound(all(arr), make_pair((ll)i, 0LL)) - arr.begin();
