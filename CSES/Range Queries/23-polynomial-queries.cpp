...
node tree_op(node a, node b) {
    return {a.sum + b.sum, a.count + b.count, a.idx_sum + b.idx_sum};
}
node tree_e() { return {0, 0, 0}; }
// merge lazy_node in node
node tree_mapping(lazy_node f, node s) {
    return {
        s.sum + f.coeff * s.idx_sum + f.constant * s.count, 
        s.count, 
        s.idx_sum
    };
}
...
lazy_node tree_composition(lazy_node f, lazy_node s) {
    return {f.coeff + s.coeff, f.constant + s.constant};
}
lazy_node tree_id() {return {0, 0};}
...
int main() {
    ...
    lazy_segtree_op tree(arr);

    while(q-- > 0) {
        cin >> query_t >> a >> b;
        if(query_t == 1) {
            tree.apply(a, b + 1, lazy_node{1, 1 - a});
        } else {
            cout << tree.prod(a, b + 1).sum << "\n"; 
        }
    }
    return 0;
}