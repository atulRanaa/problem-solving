...
Node op(Node a, Node b) {
    ...
    parent.suff = max(b.suff, a.suff + b.sum);
    parent.max_subarray = max({a.max_subarray, b.max_subarray, a.suff + b.pref});

    return parent;
};

Node e() { return {0, 0, 0, 0};}
using segtree_op = segtree<Node, op, e>;
...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        ...
        arr[i] = {x, max(0LL, x), max(0LL, x), max(0LL, x)};
    }
    segtree_op tree(arr);
    ...
    for(int i = 1; i <= q; i++) {
        ...
        // tree.set(a, {b, max(0LL, b), max(0LL, b), max(0LL, b)});
        cout << tree.prod(a, b + 1).max_subarray << "\n";
    }
    return 0;
}