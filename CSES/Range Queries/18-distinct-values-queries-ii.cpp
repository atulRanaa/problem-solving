...
int main() {
    ...
    int end_index = n + 5;
    vector<ll> ind_arr(n + 1, end_index);
    map< ll, set<ll> > ind_store;
    {
        map<ll, ll> exists;
        for(int i = 1; i <= n; i++) {
            auto it = exists.find(arr[i]);

            if(it != exists.end()) {
                ind_arr[it->second] = i;
            }
            exists[arr[i]] = i;
            ind_store[arr[i]].insert(i);
        }
    }


    segtree_min tree(ind_arr);
    int query_t, a, b;
    for(int i = 0; i < q; i++) {
        cin >> query_t >> a >> b;
        ...
        if(query_t == 1) {
            ...
            auto& old_set = ind_store[ele];
            auto& new_set = ind_store[b];

            auto it = old_set.find(a);
            if(it == old_set.begin()) {
                // (x1), x2, x3, x4
                // only process next
            } else {
                ...
                ind_arr[prev_it] = ((next_it == old_set.end())?end_index:*next_it);
                tree.set(prev_it, ind_arr[prev_it]);
            }
            ...
            if(it == new_set.begin()) {
                // (x1), x2
                // x1, (x2)
                ind_arr[a] = ((next(it) == new_set.end())?end_index:*next(it));
                tree.set(a, ind_arr[a]);
            } else {
                // x1, (x2), x3
                int prev_index = *prev(it);
                ind_arr[prev_index] = a;
                tree.set(prev_index, ind_arr[prev_index]);
            ...
            // anser if [a, b] distinct;
            int min_idx = tree.prod(a, b + 1);
            if(min_idx > b) {
                cout << "YES\n";
            } else {
                cout << "NO\n";
