...
ll solve(string &num, int i, int prev_d, bool lz, bool tight) {
    if(i == 0)
        return 1;

    if (dp[i][prev_d][lz][tight] != -1)
        return dp[i][prev_d][lz][tight];
    int n = num.size();
    int max_d = tight ? (num[n - i] - '0') : 9; 
    ll ans = 0;
    for(int d = 0; d <= max_d; d++) {
        if(d == prev_d && !lz)
            continue;

        ans += solve(num, i-1, d, lz && d == 0, tight && d == max_d);
    }

    return dp[i][prev_d][lz][tight] = ans;
}
...
int main() {
    ...
    a = solve(a_str, a_str.size(), -1, true, true);

    memset(dp, -1, sizeof(dp));
    b = solve(b_str, b_str.size(), -1, true, true);
