...
int main() {
    ...
    for(int i = 0; i < n; i++) {
        int a, b, c, d;
        cin >> a >> b >> c >> d;
        lines.emplace_back(min(a, c), min(b, d), max(a, c), max(b, d));
    }
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(a == c) {
            // vertical line
            events.emplace_back(a, 2, b, d);
        } else {
            // horizontal line
            events.emplace_back(a, 3, b, 0);
            events.emplace_back(c, 1, b, 0);
        }
    }
