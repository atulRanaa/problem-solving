...
int main() {
    ...
    ll ans = LLONG_MAX;
    set< pair<ll, ll> > active_set;
    int itr = 0;

    for(int i = 0; i < n; i++) {
        ll d = ceil(sqrt(ans));

        while(itr < i && arr[i].first - arr[itr].first >= d) {
            active_set.erase({arr[itr].second, arr[itr].first});
            itr++;
        }

        auto l_itr = active_set.lower_bound({arr[i].second - d, LLONG_MIN});
        auto r_itr = active_set.upper_bound({arr[i].second + d, LLONG_MAX});


        for(; l_itr != r_itr; l_itr++) {
            ll a = l_itr->second - arr[i].first;
            ll b = l_itr->first - arr[i].second;
            ll d = a * a + b * b;

            ans = min(ans, d);
        }

        active_set.insert({arr[i].second, arr[i].first});
    }
