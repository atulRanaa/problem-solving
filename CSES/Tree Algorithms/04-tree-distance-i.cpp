...
int main() {
    ...
    vector< int> ans(n + 1), down(n + 1), up(n + 1), best1(n + 1), best2(n + 1), child(n + 1);
    ...
    function<void(int, int)> dfs2 = [&](int u, int parent){
        ...
        for(int v: adj[u]) {
            ...
            int parent_contribution = (v == child[u]) ? best2[u] : best1[u];
            up[v] = max(up[u], parent_contribution) + 1;
