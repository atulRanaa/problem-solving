...
int dfs(int u, int parent, vector< vector<int> > &adj, vector<int> &visited, int &ans) {
    ...
    int n = child.size();
    if(n > 0) {
        ans = max(ans, child[n-1] + 1);
    }
    if(n > 1) {
        ans = max(ans, child[n-1] + child[n-2] + 1);
    }

    return ((n==0)?0:child[n-1]) + 1;
}
