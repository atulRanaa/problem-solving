...
node e_min() { return {0, inf}; }   // use a large sentinel, e.g. const ll inf = 1e18;
using segtree_min = segtree<node, op_min, e_min>;
...
int main() {
    ...
    vector<ll> depth(n + 1), in(n + 1), out(n + 1);
    vector<node> tour;
    int d = 0;
    ...
    segtree_min tree(tour);
    while(q-- > 0) {
        cin >> a >> b;

        int l = min(in[a], in[b]);
        int r = max(in[a], in[b]);
        auto result = tree.prod(l, r + 1);
        int c = result.id;
